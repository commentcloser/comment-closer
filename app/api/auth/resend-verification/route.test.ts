...
const { consumeRateLimit, recordFailedAttempt, isRateLimited, findUnique, deleteMany, create, sendVerificationEmail } = vi.hoisted(() => ({
...
describe('resend-verification rate limiting', () => {
  it('consumes an atomic token per IP and per canonical email, not the check-then-act pair', async () => {
    ...
    expect(keys).toContain('resend-ip:1.2.3.4');
    // +tag and gmail dots collapse to victim@gmail.com — one shared bucket.
    expect(keys).toContain('resend:victim@gmail.com');
